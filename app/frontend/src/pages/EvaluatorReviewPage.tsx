      ...
      <div className="mx-auto grid max-w-[1680px] gap-4 p-4 lg:grid-cols-[minmax(0,.9fr)_minmax(480px,1.1fr)] md:p-6">
        ...
        <section className="min-w-0 rounded-panel bg-white p-5 soft-edge">
          ...
          <div className="mt-5 rounded-panel bg-[#201f1d] p-5 text-white shadow-xl">
            <div className="grid gap-3 sm:grid-cols-3"><div><p className="m-0 text-micro text-white/60">Case outcome · threshold {definition.rubric.caseThreshold}</p><p className="m-0 mt-1 font-semibold">{summary?.caseScore} · {summary?.casePassed ? "Meets threshold" : "Below threshold"}</p></div><div><p className="m-0 text-micro text-white/60">AI interaction · threshold {definition.rubric.interactionThreshold}</p><p className="m-0 mt-1 font-semibold">{summary?.interactionScore} · {summary?.interactionPassed ? "Meets threshold" : "Below threshold"}</p></div><div><p className="m-0 text-micro text-white/60">Calculated</p><p className="m-0 mt-1 font-semibold">{summary ? outcomeLabel(summary.outcome) : ""}</p></div></div>
