...
const caseMarkdown = readFileSync(
  resolve(__dirname, "../src/content/simulations/first-year-associate-ma-due-diligence/case.md"),
  "utf-8"
);

const evaluationMarkdown = readFileSync(
  resolve(__dirname, "../src/content/simulations/first-year-associate-ma-due-diligence/evaluation.md"),
  "utf-8"
);
...
async function seed() {
  ...
  await setDoc(doc(db, "simulations", simulation.slug), simulation);
  console.log(`✅ Seeded: ${simulation.slug}`);
  process.exit(0);
}
